...
class GandalfClient(object):
    ...
    def __init__(self, host, port, request=None):
        self.host = host
        self.port = port
        self.request = request or self._request_method
    ...
    def _request(self, *args, **kwargs):
        route = kwargs.pop('url')
        kwargs['url'] = self._get_url(route)
        return self.request(*args, **kwargs)

    def _request_method(self, *args, **kwargs):
    ...
    def repository_rename(self, old_name, new_name):
        ...
        return self._request(
            url='/repository/{0}'.format(old_name),
            method="PUT",
    ...
    def repository_archive(self, name, ref, format):
        ...
        return self._request(
            url='/repository/{0}/archive/{1}.{2}'.format(name, ref, format),
            method="GET",
        )
    ...
    def repository_contents(self, name, path):
        ...
        return self._request(
            url='/repository/{0}/contents/{1}'.format(name, path),
            method="GET",
        )
    ...
    def user_delete_key(self, name, keyname):
        ...
        return self._request(
            url='/user/{0}/key/{1}'.format(name, keyname),
            method="DELETE",
        )
